...
class _NotifyMeState extends State<NotifyMe> {
  ...
  void _subscribeToNotifications() async {
    ...
    if (_fcmToken == null) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('FCM token not available')));
      return;
    }
    ...
    if (isGoodDay) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Surf conditions are not good today')));
    }
  }

  void _sendPushNotification(String phoneNumber) {
    // Send the notification via your backend here
    print("Sending push notification to $phoneNumber.");
  }
